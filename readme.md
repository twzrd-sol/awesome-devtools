...
* [TWZRD Agent Intel](https://intel.twzrd.xyz) - Trust scoring and identity verification for AI agent wallets on Solana. Free MCP endpoint for `score_agent(wallet)` and `preflight_check(wallet)`; paid `get_trust_receipt(wallet)` via x402 micropayment.
